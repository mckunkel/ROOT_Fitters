...
void linfitMKGaus(TH1F *h33 , Double_t low, Double_t high, Double_t p0, Double_t p1, Double_t initialPar, Double_t width, Double_t factor, Int_t draw_opt){

  double nEnt = h33->GetEntries();
  TF1 *fitter = new TF1("fitter","gaus + [3] + [4]*x",low,high);
fitter->SetParameters(100, initialPar, width, p0, p1); fitter->SetParLimits(0,10,nEnt); fitter->SetParLimits(1,initialPar-width,initialPar+width);
...
TLatex *ra = new TLatex(startx*0.93, starty*0.6,Form("#frac{S}{S+B}= %.3f", ratio));
...
//  linfitMKGaus(TH1F, Double_t, Double_t, Double_t, Double_t, Double_t, Double_t, Double_t, Int_t);
