...
void fitMKGaus_wMATRIX(TH1 *h33 , Double_t low, Double_t high, Double_t p0, Double_t p1, Double_t p2, Double_t p3, Double_t initialPar, Double_t width, Double_t factor, Int_t draw_opt, TMatrixD *fit_pars ){//Double_t *array[8]
...
Double_t startx=x->GetXmin()+0.75*(x->GetXmax()-x->GetXmin());
  ...
  fit_pars->ResizeTo(8,1);
  *fit_pars(0,0) = yield;
  *fit_pars(1,0) = bckgd;
  *fit_pars(2,0) = factor;
  *fit_pars(3,0) = par[1];
  *fit_pars(4,0) = meanError;
  *fit_pars(5,0) = par[2];
  *fit_pars(6,0) = sigmaError;
  *fit_pars(7,0) =ratio;
