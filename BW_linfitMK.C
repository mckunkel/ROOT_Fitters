...
void BW_linfitMK(TH1F *h33 , Double_t low, Double_t high, Double_t p0, Double_t p1, Double_t initialPar, Double_t width, Double_t factor, Int_t draw_opt){

  double nEnt = h33->GetEntries();
  TF1 *fitter = new TF1("fitter","[0]*pow(x,2)/(pow(pow([1],2) - pow(x,2),2) + pow(x,2)*pow([2],2)) + [3] + [4]*x",low,high);
  fitter->SetParameters(240, initialPar, width, p0, p1);  fitter->SetParLimits(1,initialPar-width,initialPar+width);
  fitter->SetParLimits(0,0,nEnt);  
